...
//	options.bold = new options.cookie( 'bold' ).get( options.bold ) === 'false' ? false : true;
    ...
    sprite.update = function (optionsUpdate) {
        ...
        context.fillText(options.text, 0, canvas.height + 1 * borderThickness);
...
THREE.gui.spriteText = function (gui, sprite, guiParams, options) {
    ...
    if (options.optionsDefault === undefined) {
        ...
        if (Array.isArray(sprite))
            ...
            console.error('THREE.gui.spriteText: options.optionsDefault is undefined.');
